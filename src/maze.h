...
class SquareMaze {
    public:
        ...
        void helper(int &front, int &val, int &x, int &y, vector<bool> &visits, vector<pair<int, int>> &solved, queue<int> &qu);
        
        void resultAppender(int &len, vector<int> &result, vector<pair<int, int>> &solved);
        ...
        */
        ...
        void pathFinder(cs225::PNG*& soln, vector<int> &solves, int &x, int &y, unsigned &i);
    ...
    private:
        ...
        int width_;
        int size_;
        vector<bool> right_; // track of rights
        vector<bool> down_; // track of downs
        DisjointSets dsets_;
